...
import {
  ...
  inviteBuyerApplication,
  markBuyerApplicationUnderReview,
  rejectBuyerApplication,
  saveBuyerApplicationReview,
} from "@/lib/application-review";
import { getAdminApplicationDocuments } from "@/lib/application-documents";
import type { BuyerApplicationRow } from "@/lib/supabase/database.types";

export async function BuyerApplicationReviewPanel({
  record,
}: Readonly<{
  record: BuyerApplicationRow;
}>) {
  const documents = await getAdminApplicationDocuments("buyer", record.id);

  return (
    <div className="grid gap-6">
      <PageCard title={record.applicant_name} description="Buyer application detail and review controls.">
        ...
        <div className="mt-5 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          <Field label="Buyer type" value={record.buyer_type} />
          <Field label="Budget range" value={record.budget_range} />
          <Field label="Proof of funds" value={record.proof_of_funds_status} />
          <Field label="Urgency" value={record.urgency} />
        </div>

        <div className="mt-5 grid gap-4 md:grid-cols-2">
          <ListField label="Niches of interest" value={record.niches_of_interest.join(", ")} />
          <ListField label="Asset preferences" value={record.asset_preferences.join(", ")} />
        </div>

        <div className="mt-5 rounded-[1.5rem] border border-ink-200 bg-[rgb(var(--surface))] p-4 text-sm leading-6 text-ink-700">
          <div className="text-[11px] font-semibold tracking-[0.2em] text-ink-500 uppercase">
            Application message
          </div>
          <p className="mt-2 whitespace-pre-wrap">{record.message}</p>
        </div>
      </PageCard>
      ...
      <PageCard title="Review and invite" description="Admin-only review controls and audit trail.">
        <ApplicationReviewActions
          ...
          saveAction={saveBuyerApplicationReview}
          markUnderReviewAction={markBuyerApplicationUnderReview}
          approveAction={approveBuyerApplication}
          rejectAction={rejectBuyerApplication}
          inviteAction={inviteBuyerApplication}
      ...
      <PageCard title="Audit trail" description="Internal timestamps for review and invite history.">
        ...
        <div className="mt-4 text-sm leading-6 text-ink-600">
          <Link href="/admin/applications/buyers" className="font-semibold text-accent-700 hover:text-accent-600">
            Back to the buyer queue
          </Link>
        </div>
      </PageCard>

      <ApplicationDocumentReviewSection role="buyer" applicationId={record.id} documents={documents} />
    </div>
  );
}
