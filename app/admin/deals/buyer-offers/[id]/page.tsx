...
import {
  acceptOfferAndOpenClosingAction,
  getAssetClosingByOfferId,
...
export default async function AdminDealsBuyerOfferDetailPage({ params, searchParams }: AdminBuyerOfferDetailPageProps) {
  const record = await getAdminBuyerOfferSubmissionById(params.id);

  if (!record) {
    redirect("/admin/deals/buyer-offers?error=Submission%20not%20found.");
  }

  const internalOffer = record.offer_record_id ? await getOfferRecordById(record.offer_record_id) : null;
  const closing = record.offer_record_id ? await getAssetClosingByOfferId(record.offer_record_id) : null;
  ...
  return (
    <div className="grid gap-6">
      <AdminPageHeader
        ...
        description="Review a buyer-submitted offer, update its status, and move it into the closing desk when ready."
      />
      ...
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="grid gap-4">
              ...
              <div className="grid justify-items-end gap-2">
                ...
                {closing ? (
                  <span className="rounded-full border border-emerald-200 bg-[rgba(12,35,22,0.96)] px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-emerald-700">
                    Closing {closing.closing_status.replaceAll("_", " ")}
                  </span>
                ) : null}
              </div>
            </div>
            ...
            <div className="grid gap-3">
              <form action={updateBuyerOfferSubmissionStatusAction} className="grid gap-3">
                ...
                <input type="hidden" name="return_to" value={`/admin/deals/buyer-offers/${record.id}?saved=updated`} />
              ...
              {!record.offer_record_id ? (
                <form action={promoteBuyerOfferSubmissionAction}>
                  <input type="hidden" name="id" value={record.id} />
                  <input type="hidden" name="return_to" value={`/admin/deals/buyer-offers/${record.id}?saved=promoted`} />
                  <button
                    type="submit"
                ...
                <div className="grid gap-3">
                  <OfferCard offer={internalOffer} editable buyerOptions={buyerRecords} returnTo={`/admin/offers/${internalOffer.id}?saved=updated`} />
                  {closing ? (
                    <Link
                    ...
                    <form action={acceptOfferAndOpenClosingAction}>
                      <input type="hidden" name="offer_id" value={internalOffer.id} />
                      <input type="hidden" name="return_to" value="/admin/deals/closing-desk" />
                      <button
                        type="submit"
                        className="rounded-full border border-emerald-200 bg-[rgba(12,35,22,0.96)] px-4 py-2 text-sm font-semibold text-emerald-700 transition hover:border-emerald-300 hover:text-emerald-600"
                      >
        ...
        <div className="grid gap-6">
          <PageCard title="Links" description="Navigate to the related records and closing desk.">
            <div className="grid gap-3 text-sm leading-6 text-ink-700">
              ...
              {closing ? (
                <Link
                  href={`/admin/deals/closing-desk/${closing.id}`}
                  className="rounded-2xl border border-emerald-200 bg-[rgba(12,35,22,0.96)] px-4 py-3 font-semibold text-emerald-700 transition hover:border-emerald-300 hover:text-emerald-600"
                >
                  Open closing desk
                </Link>
              ) : null}
          ...
          {internalOffer ? (
            <PageCard title="Internal offer row" description="The bridged internal offer record created from this submission.">
              <OfferCard
                ...
                returnTo={`/admin/deals/buyer-offers/${record.id}?saved=updated`}
              />
            </PageCard>
          ) : null}
        </div>
