...
export default async function AdminDealsBuyerOffersPage({ searchParams }: AdminBuyerOffersPageProps) {
  ...
  return (
    <div className="grid gap-6">
      <AdminPageHeader
        title="Buyer Offers"
        description="Controlled buyer offer submissions that can be promoted into the closing workflow."
      />
      ...
      <PageCard
        title="Offer queue"
        description="Filter buyer-submitted offers and open the offer or closing desk when the record is ready."
      >
        <div className="flex flex-wrap items-center gap-3">
          {filterOptions.map((option) => (
            <Link
              key={option.value}
              href={option.value === "all" ? "/admin/deals/buyer-offers" : `/admin/deals/buyer-offers?status=${option.value}`}
              className={[
