...
import { BuyerApplicationReviewPanel } from "@/components/buyer-application-review-panel";
import { HistoryFeed } from "@/components/history-feed";
import { PageCard } from "@/components/page-card";
import { getApplicationHistoryEvents } from "@/lib/history";
import { getBuyerApplicationRecord } from "@/lib/application-review";

type BuyerApplicationDetailPageProps = Readonly<{
  params: {
    id: string;
  };
  searchParams?: {
...
export default async function BuyerApplicationDetailPage({
  params,
  searchParams,
}: BuyerApplicationDetailPageProps) {
  const record = await getBuyerApplicationRecord(params.id);
  ...
  return (
    <div className="grid gap-6">
      <AdminPageHeader
        ...
        description="Review, approve, reject, and invite approved buyer applicants from this private admin view."
      />
      ...
      {searchParams?.saved ? (
        <div className="rounded-[1.5rem] border border-accent-200 bg-[rgb(var(--accent-soft))] px-5 py-4 text-sm font-medium text-accent-800">
          Buyer application {searchParams.saved.replaceAll("_", " ")} successfully.
        </div>
      ) : null}
      ...
      {searchParams?.document_saved ? (
        <div className="rounded-[1.5rem] border border-accent-200 bg-[rgb(var(--accent-soft))] px-5 py-4 text-sm font-medium text-accent-800">
          Buyer document review saved successfully.
        </div>
      ) : null}
      {searchParams?.error ? (
      ...
      <BuyerApplicationReviewPanel record={record} />

      <PageCard title="History" description="Derived application activity with safe timestamps and event labels.">
        <HistoryFeed events={await getApplicationHistoryEvents("buyer", record.id)} compact />
      </PageCard>
    </div>
  );
}
