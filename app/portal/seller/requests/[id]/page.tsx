...
import {
  acknowledgeSellerPortalRequestAction,
  completeSellerPortalRequestAction,
  getSellerPortalRequestById,
  getSellerPortalRequestDocuments,
...
export default async function SellerRequestDetailPage({ params, searchParams }: SellerRequestDetailPageProps) {
  ...
  const request = record.status === "activated" ? await getSellerPortalRequestById(record.id, params.id) : null;

  if (!request) {
    redirect("/portal/seller/requests?error=Request%20not%20found.");
  }

  const requestDocs = await getSellerPortalRequestDocuments(request.id);
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6 xl:grid-cols-[1.04fr_0.96fr]">
        <PageCard title="Request summary" description="Only safe seller-facing request details are shown here.">
          <RequestCard record={request} href={portalRequestDetailRoute("seller", request.id)} />
          ...
          <div className="mt-5 flex flex-wrap items-center gap-3">
            {canAcknowledge ? (
              <form action={acknowledgeSellerPortalRequestAction}>
            ...
            {canComplete ? (
              <form action={completeSellerPortalRequestAction}>
                <input type="hidden" name="id" value={request.id} />
                <button
                  type="submit"
        ...
        <div className="grid gap-6">
          ...
          <PageCard title="History" description="Safe request activity with timestamps and sanitized labels.">
            <HistoryFeed events={await getRequestHistoryEvents("seller", request.id)} compact />
          </PageCard>
        </div>
      </div>
    </PortalShell>
