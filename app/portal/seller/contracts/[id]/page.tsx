...
type SellerContractDetailPageProps = Readonly<{
  params: {
    id: string;
  };
}>;

export default async function SellerContractDetailPage({ params }: SellerContractDetailPageProps) {
  const record = await requireActivatedSellerPortalAccess();
  const contract = await getSellerPortalContractById(record.id, params.id);

  if (!contract) {
    redirect("/portal/seller/contracts?error=Contract%20not%20found.");
  }

  const historyEvents = await getContractHistoryEvents("seller", contract.id);

  return (
    <PortalShell
      eyebrow="Seller portal"
      title={contract.asset_name}
      description="Seller-safe contract detail with controlled status and next-step labels."
      role="seller"
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="grid gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {contract.transfer_row_id ? (
                <Link
                  href={`/portal/seller/transfers/${contract.transfer_row_id}`}
        ...
        <PageCard title="What the seller can see" description="The contract surface stays safe and minimal.">
        ...
        <PageCard title="History" description="Safe seller-side contract activity with sanitized labels and timestamps.">
