...
type BuyerTransferDetailPageProps = Readonly<{
  params: {
    id: string;
  };
}>;
...
export default async function BuyerTransferDetailPage({ params }: BuyerTransferDetailPageProps) {
  ...
  if (!transfer) {
    redirect("/portal/buyer/transfers?error=Transfer%20not%20found.");
  }

  const historyEvents = await getTransferHistoryEvents("buyer", transfer.id);

  return (
    <PortalShell
      eyebrow="Buyer portal"
      title={transfer.asset_name}
      description="Buyer-safe transfer detail with controlled status and closeout labels."
      role="buyer"
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="grid gap-4">
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              {transfer.contract_id ? (
                <Link
                  href={`/portal/buyer/contracts/${transfer.contract_id}`}
        ...
        <PageCard title="What the buyer can see" description="The transfer surface stays safe and minimal.">
          <div className="grid gap-3 text-sm leading-6 text-ink-700">
            ...
            <div>Key dates and linked contract reference</div>
            <div>Archive status only after completion</div>
            <div>No internal notes or private seller data</div>
          </div>
        </PageCard>

        <PageCard title="History" description="Safe buyer-side transfer activity with sanitized labels and timestamps.">
