...
type BuyerNotificationDetailPageProps = Readonly<{
  params: {
    kind: string;
    id: string;
  };
...
export default async function BuyerNotificationDetailPage({ params }: BuyerNotificationDetailPageProps) {
  ...
  return (
    ...
    >
      <div className="grid gap-6">
          ...
          {notification.kind === "request" && request ? (
            <div className="grid gap-4 text-sm leading-6 text-ink-700">
              ...
              <Link
                ...
                className="inline-flex w-fit items-center justify-center rounded-full border border-accent-200 bg-[rgba(31,26,18,0.96)] px-4 py-2 text-sm font-semibold text-accent-700 transition hover:border-accent-300 hover:text-accent-600"
              >
                Open request
              </Link>
            </div>
            ...
            <div className="grid gap-4">
              ...
              <LifecycleTimeline
                ...
                description={deal.buyer_status_label}
                events={buildLifecycleTimelineEvents(deal, "buyer")}
              />
              <Link
                href={notification.href}
                className="inline-flex w-fit items-center justify-center rounded-full border border-accent-200 bg-[rgba(31,26,18,0.96)] px-4 py-2 text-sm font-semibold text-accent-700 transition hover:border-accent-300 hover:text-accent-600"
        ...
        <PageCard title="Back" description="Return to the buyer notification center.">
          <Link
            href={portalNotificationCenterRoute("buyer")}
